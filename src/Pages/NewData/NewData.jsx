                ...
                <form method="post" className="flex flex-wrap max-w-[500px] justify-between" onSubmit={submitData}>
                    {props.cols.filter((element) => element.field !== "id" && element.field !== "created").map(col => (
                        <div key={col.id} className="item w-[47%] flex flex-col gap-[10px] mb-[20px]">
                            ...
                            {col.type === 'boolean' ? (
                            <input className="p-[10px] bg-transparent text-[color:var(--btn-bg)] outline-none border-[color:var(--soft-text)] border-solid border-[1px] rounded-[3px]" onChange={handleChange} name={col.field} type="checkbox" checked={formData[col.field] || false} placeholder={col.placehold} />
                            ) : (<input className="p-[10px] bg-transparent text-[color:var(--btn-bg)] outline-none border-[color:var(--soft-text)] border-solid border-[1px] rounded-[3px]" onChange={handleChange} name={col.field} type={col.type} value={formData[col.field] || ''} placeholder={col.placehold} />)}
                            </div>
                    ))}
